...
void Haamin_Remote(uint16_t action, uint8_t workMode)
{
        ...
        if(workMode == MODE_NORMAL)
        {
            ...
            else if(action == MENU_KEY)
            {
                ...
                date_Mode = SET_YEAR;
                dateValue = jdate_Year;
                dateMounth_t = jdate_Mounth;
                dateDay_t = jdate_Day;
        ...
        else if(workMode == DATE_SETTING)
        {
            ...
            else if (action == OK_KEY)
            {
                ...
                else if(date_Mode == SET_DOW)
                {
                    date_Week = dateValue;
                    jdate_Day = dateDay_t;
                    jdate_Mounth = dateMounth_t;
                    jdate_Year = dateYear_t;
                    jalali_to_gregorian((1400+jdate_Year),jdate_Mounth,jdate_Day,miladi_Calender);
